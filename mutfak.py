...
def create_table(tablo):
    ...
    sql = """CREATE TABLE %s (
             ...
             AD    VARCHAR(30) ,
             SOYAD VARCHAR(30) ,  
             TC_NO    VARCHAR(30) PRIMARY KEY)""" %( tablo )
    try:
        cursor.execute("DROP TABLE IF EXISTS %s " % tablo)
        cursor.execute(sql)
        print 'veritabanında tablo oluşturuldu'
    except:
        print 'veritabanında tablo oluşturulamadı'
...
def db_csv_gonder(tablo , dosya_yolu):
    ...
    if k==0:     print 'csv kayıtları veritabanına yüklendi'
