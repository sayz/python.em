...
def hakkinda():
    ...
    url.pack()
    def tikla(env=None):    webbrowser.open('http://kil.bil.omu.edu.tr')
    def ust(env=None):      url["fg"] = "darkblue"
    def terket(env=None):   url["fg"]="#660000"
    url.bind("<Button-1>",tikla)
    url.bind("<Enter>",ust)
    url.bind("<Leave>",terket)

Button( text="hakkında" , fg="#660000" ,width=6,height=1,
        command = hakkinda).grid(row=7,column=1,columnspan=2)
